...
cd src
call npx -y staticrypt index.html -d .. -p "%PW%" --remember 30 --short --template-title "HLE 통합 분석 허브" --template-instructions "한화생명e스포츠 내부 전용 페이지입니다. 디스코드에 공유된 비밀번호를 입력하세요." --template-placeholder "비밀번호" --template-button "접속" --template-remember "이 기기에서 30일간 기억" --template-color-primary "#f37321"
cd ..
